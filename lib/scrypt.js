...
async function scrypt (key, salt, N, r, p, dkLen, progressCallback) {
  ...
  if (progressCallback) {
    var totalOps = p * N * 2
    var currentOp = 0
    var progressEvery = parseInt(totalOps/200.0, 10);

    tickCallback = function () {
      ++currentOp

      // send progress notifications once every progressEvery ops
      if (currentOp % progressEvery === 0) {
        return {
          current: currentOp,
          total: totalOps,
